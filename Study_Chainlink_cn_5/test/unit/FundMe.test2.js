...
describe("测试主题:测试FundMe合约的函数", async function () {
  ...
  it("单元测试用例9:达到目标值,窗口已关闭,存放金额已达目标值,调用 getFund 函数来提款.", async function () {
    ...
    await helpers.time.increase(500);
    await helpers.mine();
    await expect(fundMe.connect(await ethers.getSigner(firstAccount)).getFund())
  ...
  it("单元测试用例13:窗口已关闭，目标未到达，资助者有余额.", async function () {
    ...
    await fundMe.connect(deployer).fund({ value: ethers.parseEther("0.1") });
    ...
    await expect(fundMe.connect(deployer).refund())
      .to.emit(fundMe, "RefundByFunder")
      .withArgs(deployer.address, ethers.parseEther("0.1"));
  });
});
